...
impl MemTable {
    ...
    pub fn put(&self, key: &[u8], value: &[u8]) -> Result<()> {
        ...
        // TODO: may want to account for when you insert a duplicate by subtracting size of prev kv
...
impl StorageIterator for MemTableIterator {
    ...
    fn next(&mut self) -> Result<()> {
        let next_item = self.with_iter_mut(|iter| match iter.next() {
            Some(entry) => (entry.key().clone(), entry.value().clone()),
            None => (Bytes::new(), Bytes::new()),
        });

        self.with_mut(|fields| *fields.item = next_item);
